...
# ══════════════════════════════════════════════════════════════════
# BACKTEST GROWTH/VALUE — buy & hold de largo plazo
# Distinto del backtest de swing (mean reversion, días de holding):
# aquí se compran las empresas con mejor fund_score y valoración
# razonable (P/E bajo mediana del sector), se mantienen 1-2 años SIN
# vender por stop/target intradía, y se compara el retorno medio de
# la cartera contra el SPY en el mismo periodo. Múltiples entradas
# trimestrales a lo largo de 2 años para tener muestra estadística
# (no una sola fecha de entrada).
#
# NOTA DE SESGO (igual que el resto del backtest): fund_score y pe
# son el valor ACTUAL del CSV aplicado retroactivamente — no tenemos
# fundamentales históricos reales por trimestre. Es una simplificación
# aceptada explícitamente para este análisis.
# ══════════════════════════════════════════════════════════════════

def download_spy_precios(start_date, end_date):
    """Precio de cierre diario de SPY, para calcular el retorno del benchmark."""
    try:
        ...
        if isinstance(c, pd.DataFrame):
            c = c.iloc[:, 0]
        return {str(d)[:10]: float(p) for d, p in zip(raw['Date'], c.astype(float))}
    except Exception as e:
        print(f"  ⚠ Error descargando SPY: {e}")
        return {}


def precio_en_o_despues(precios_dict, fecha_str, max_dias_busqueda=10):
    """Busca el precio de cierre en fecha_str, o el primer día hábil
    posterior disponible (hasta max_dias_busqueda) si esa fecha exacta
    no tiene dato (findes, festivos)."""
    fecha = datetime.strptime(fecha_str, '%Y-%m-%d')
    for i in range(max_dias_busqueda):
        f = (fecha + timedelta(days=i)).strftime('%Y-%m-%d')
        if f in precios_dict:
            return precios_dict[f], f
    return None, None


def seleccionar_cartera_growth_value(fecha_entrada: str, fund_scores: dict,
                                       prices: dict, min_score: float, max_n: int = None):
    """
    Selecciona las candidatas para una fecha de entrada dada:
      - fund_score >= min_score
      - pe disponible y por debajo de la mediana del sector (valoración
        razonable, sin mirar momentum/precio de la acción en sí)
    La mediana de cada sector se calcula "leave-one-out" (excluyendo al
    propio ticker evaluado) para no penalizar sistemáticamente a sectores
    con pocos representantes tras el filtro de score — con un único
    miembro, comparar su PE contra "la mediana de sí mismo" siempre lo
    excluiría, lo cual no tiene sentido.
    Si un sector queda con 0 comparables tras excluir al propio ticker
    (sector con un solo miembro), se usa la mediana global de PE entre
    todas las candidatas como referencia de respaldo.
    Devuelve lista de tickers ordenada por fund_score descendente,
    limitada a max_n si se especifica.
    """
    candidatas_score = {t: d for t, d in fund_scores.items()
                        if d['fund_score'] >= min_score and d.get('pe') is not None and d['pe'] > 0}

    if not candidatas_score:
        return []

    pe_por_sector = {}
    for t, d in candidatas_score.items():
        pe_por_sector.setdefault(d['sector'], []).append((t, d['pe']))

    mediana_global = float(np.median([d['pe'] for d in candidatas_score.values()]))

    seleccion = []
    for t, d in candidatas_score.items():
        if t not in prices:
            continue
        peers = [pe for tk, pe in pe_por_sector[d['sector']] if tk != t]
        mediana_referencia = float(np.median(peers)) if peers else mediana_global
        if d['pe'] >= mediana_referencia:
            continue  # valoración no razonable (P/E por encima de la mediana de sus comparables)
        seleccion.append((t, d['fund_score'], d['pe'], d['sector']))

    seleccion.sort(key=lambda x: x[1], reverse=True)
    if max_n:
        seleccion = seleccion[:max_n]
    return seleccion


def calcular_retorno_ticker(prices_df: pd.DataFrame, fecha_entrada: str, dias_horizonte: int):
    """
    Retorno de un ticker desde fecha_entrada hasta fecha_entrada + dias_horizonte
    (aprox. en días naturales; se usa el precio de cierre más próximo
    disponible en cada extremo). Devuelve None si no hay datos suficientes.
    """
    df = prices_df.copy()
    df['Date'] = pd.to_datetime(df['Date'])
    fecha_dt = pd.to_datetime(fecha_entrada)
    fecha_fin_dt = fecha_dt + timedelta(days=dias_horizonte)

    df_entrada = df[df['Date'] >= fecha_dt]
    if df_entrada.empty:
        return None
    precio_entrada = float(df_entrada.iloc[0]['Close'])
    fecha_entrada_real = df_entrada.iloc[0]['Date']

    df_fin = df[df['Date'] >= fecha_fin_dt]
    if df_fin.empty:
        # No hay suficientes datos futuros (p.ej. entrada muy reciente) — sin resolver aún
        return None
    precio_fin = float(df_fin.iloc[0]['Close'])
    fecha_fin_real = df_fin.iloc[0]['Date']

    retorno_pct = (precio_fin - precio_entrada) / precio_entrada * 100
    return {
        'precio_entrada': round(precio_entrada, 2),
        'precio_fin':      round(precio_fin, 2),
        'fecha_entrada_real': str(fecha_entrada_real)[:10],
        'fecha_fin_real':     str(fecha_fin_real)[:10],
        'retorno_pct':        round(retorno_pct, 2),
    }


def run_growth_value_backtest(min_score=7.5, max_n=20, years_back=2,
                                horizontes_dias=(365, 730), freq_meses=3):
    """
    Backtest growth/value con múltiples entradas trimestrales:
      1. Genera fechas de entrada cada `freq_meses` meses, desde hace
         `years_back` + 2 años (para que incluso la entrada más reciente
         tenga margen de sobra para resolver a +2 años) hasta hace 2 años
         (último punto con datos suficientes para medir +2 años completos).
      2. En cada fecha, selecciona la cartera growth/value (fund_score
         actual >= min_score, PE bajo mediana del sector).
      3. Mide el retorno de cada ticker de la cartera a +1 año y +2 años.
      4. Compara el retorno medio de la cartera contra el retorno de SPY
         en exactamente el mismo periodo (mismo par de fechas).
    """
    print(f"""
  ══════════════════════════════════════════════════════════════
  BACKTEST GROWTH/VALUE — Buy & Hold de largo plazo
  min_fund_score >= {min_score} · PE < mediana sector · top {max_n or 'sin límite'}
  Horizontes: {[h//365 for h in horizontes_dias]} año(s) · Entradas cada {freq_meses} meses
  ══════════════════════════════════════════════════════════════""")

    fund_scores = load_fundamental_scores()
    if not fund_scores:
        print("  ❌ Sin CSV de fundamentales — no se puede ejecutar este backtest")
        return None

    tickers = load_tickers()

    # Necesitamos precios desde hace (years_back + max_horizonte_años) hasta hoy,
    # para que la entrada más antigua tenga datos suficientes para resolver +2 años.
    max_horizonte_anios = max(horizontes_dias) / 365
    hoy = datetime.today()
    start_precios = (hoy - timedelta(days=365*(years_back + max_horizonte_anios + 0.2))).strftime('%Y-%m-%d')
    end_precios = hoy.strftime('%Y-%m-%d')

    print(f"\n  Descargando precios ({start_precios} → {end_precios})...")
    prices = download_prices(tickers, start_date=start_precios, end_date=end_precios)
    if not prices:
        print("  ❌ Sin precios descargados")
        return None

    print(f"  Descargando SPY (benchmark)...")
    spy_precios = download_spy_precios(start_precios, end_precios)
    spy_df = pd.DataFrame({'Date': list(spy_precios.keys()), 'Close': list(spy_precios.values())})

    # ── Generar fechas de entrada trimestrales ──────────────────────
    # Última entrada posible: hoy - max_horizonte (para que pueda haber
    # resuelto ya el horizonte más largo). Primera entrada: years_back
    # atrás desde esa última fecha.
    ultima_entrada = hoy - timedelta(days=365*max_horizonte_anios)
    primera_entrada = ultima_entrada - timedelta(days=365*years_back)

    fechas_entrada = []
    f = primera_entrada
    while f <= ultima_entrada:
        fechas_entrada.append(f.strftime('%Y-%m-%d'))
        f += timedelta(days=30*freq_meses)

    print(f"\n  {len(fechas_entrada)} fechas de entrada: {fechas_entrada[0]} → {fechas_entrada[-1]}")

    resultados_por_horizonte = {h: [] for h in horizontes_dias}

    for fecha_entrada in tqdm(fechas_entrada, desc="  Simulando entradas trimestrales"):
        cartera = seleccionar_cartera_growth_value(fecha_entrada, fund_scores, prices, min_score, max_n)
        if not cartera:
            continue
        ...
        for horizonte in horizontes_dias:
            ...
            for ticker, score, pe, sector in cartera:
                r = calcular_retorno_ticker(prices[ticker], fecha_entrada, horizonte)
                if r is None:
                    continue
                retornos_cartera.append(r['retorno_pct'])
                detalle_tickers.append({'ticker': ticker, 'fund_score': score, 'pe': pe,
                                        'sector': sector, **r})

            if not retornos_cartera:
                continue

            r_spy = calcular_retorno_ticker(spy_df, fecha_entrada, horizonte)
            retorno_spy = r_spy['retorno_pct'] if r_spy else None

            retorno_medio_cartera = float(np.mean(retornos_cartera))
            resultados_por_horizonte[horizonte].append({
                'fecha_entrada':        fecha_entrada,
                'n_tickers':            len(retornos_cartera),
                'retorno_medio_cartera': round(retorno_medio_cartera, 2),
                'retorno_mediano_cartera': round(float(np.median(retornos_cartera)), 2),
                'retorno_spy':          round(retorno_spy, 2) if retorno_spy is not None else None,
                'alpha_vs_spy':         round(retorno_medio_cartera - retorno_spy, 2) if retorno_spy is not None else None,
                'pct_batieron_spy':     round(sum(1 for r in retornos_cartera if retorno_spy is not None and r > retorno_spy) / len(retornos_cartera) * 100, 1) if retorno_spy is not None else None,
                'tickers': detalle_tickers,
            })

    # ── Resumen agregado por horizonte ──────────────────────────────
    print(f"\n  ══════════════════════════════════════════════════════════════")
    print(f"  RESUMEN — GROWTH/VALUE vs SPY")
    print(f"  ══════════════════════════════════════════════════════════════")

    resumen_final = {}
    for horizonte, entradas in resultados_por_horizonte.items():
        años = horizonte / 365
        if not entradas:
            print(f"\n  [+{años:.0f} año(s)] Sin entradas resueltas todavía (necesitan más historial de precios)")
            continue

        retornos_cartera_todas = [e['retorno_medio_cartera'] for e in entradas]
        retornos_spy_todas = [e['retorno_spy'] for e in entradas if e['retorno_spy'] is not None]
        alphas = [e['alpha_vs_spy'] for e in entradas if e['alpha_vs_spy'] is not None]

        media_cartera = float(np.mean(retornos_cartera_todas))
        media_spy = float(np.mean(retornos_spy_todas)) if retornos_spy_todas else None
        media_alpha = float(np.mean(alphas)) if alphas else None
        pct_periodos_bate_spy = sum(1 for a in alphas if a > 0) / len(alphas) * 100 if alphas else None

        print(f"\n  [+{años:.0f} año(s)] — {len(entradas)} punto(s) de entrada evaluados")
        print(f"     Retorno medio cartera : {media_cartera:+.1f}%")
        print(f"     Retorno medio SPY     : {media_spy:+.1f}%" if media_spy is not None else "     Retorno medio SPY     : N/D")
        print(f"     Alpha medio (cartera-SPY): {media_alpha:+.1f}pp" if media_alpha is not None else "")
        print(f"     % periodos que baten SPY : {pct_periodos_bate_spy:.0f}%" if pct_periodos_bate_spy is not None else "")

        resumen_final[f"{años:.0f}y"] = {
            'n_entradas':              len(entradas),
            'retorno_medio_cartera':   round(media_cartera, 2),
            'retorno_medio_spy':       round(media_spy, 2) if media_spy is not None else None,
            'alpha_medio_pp':          round(media_alpha, 2) if media_alpha is not None else None,
            'pct_periodos_bate_spy':   round(pct_periodos_bate_spy, 1) if pct_periodos_bate_spy is not None else None,
            'detalle_por_entrada':     entradas,
        }

    out_dir = Path(__file__).parent / "data" / "master"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / "growth_value_backtest.json"
    with open(out_path, 'w', encoding='utf-8') as f:
        json.dump({
            'generado_en': datetime.now().isoformat(),
            'parametros': {'min_score': min_score, 'max_n': max_n, 'years_back': years_back,
                           'horizontes_dias': list(horizontes_dias), 'freq_meses': freq_meses},
...
if __name__ == '__main__':
    ...
    p.add_argument('--growth',      action='store_true',
                    help='Backtest growth/value de largo plazo (buy & hold 1-2 años, '
                         'no swing): fund_score alto + PE bajo mediana sector, '
                         'entradas trimestrales, comparado contra SPY.')
    p.add_argument('--min-score',   type=float, default=7.5,
                    help='fund_score minimo para el backtest growth/value (default 7.5)')
    p.add_argument('--max-n',       type=int, default=20,
                    help='Numero maximo de tickers en la cartera growth/value (default 20)')
    p.add_argument('--years-back',  type=int, default=2,
                    help='Años hacia atras para generar entradas trimestrales (default 2)')
    args = p.parse_args()

    if args.regimen:
        run_regime_analysis()
    elif args.growth:
        run_growth_value_backtest(min_score=args.min_score, max_n=args.max_n,
